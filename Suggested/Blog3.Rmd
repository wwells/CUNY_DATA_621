...
The [keras package](https://tensorflow.rstudio.com/keras/) is wonder library that leverages Keras to let a user build deep learning networks very easily in R.   The Keras package is designed to be a user-friendly interface and support backends like Tensorflow, Caffee, or Lasagne for doing deep learning with a variety of hardware including GPUS or TPUS.  
...
Deep learning excells at unlocking the secrets of datasets when confronted with large datasets for images, time-series, audio, video, etc.   However - it can also be used for regression!
...
Here we'll load our data (it comes with the Keras package already split into train and test) and use the data explorer package to plot our predictors against our targets. 
